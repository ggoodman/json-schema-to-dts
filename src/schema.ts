      ...
      try {
        ...
        if (failures.length >= this.validators.length) {
          throw ctx.assertionFailure(
            "anyOf",
            value,
            `Value did not match any valid sub-schema`
          );
        }
      }
    }
  ...
  assert(ctx: IValidatorContext, value: unknown) {
    if (typeof value !== "boolean") {
      throw ctx.assertionFailure(
        "type",
        value,
        `Expected value to be of type "boolean", received ${JSON.stringify(
          typeof value
        )}`
      );
    }
  ...
  assert(ctx: IValidatorContext, value: unknown) {
    ...
    if (isInteger(o.maximum) && value > o.maximum) {
      throw ctx.assertionFailure(
        "maximum",
        value,
        `Expected value to be a maximum of ${JSON.stringify(
          o.maximum
        )} but got ${JSON.stringify(value)}`
      );
    }
    ...
    if (isInteger(o.minimum) && value < o.minimum) {
      throw ctx.assertionFailure(
        "minimum",
        value,
        `Expected value to be a minimum of ${JSON.stringify(
          o.minimum
        )} but got ${JSON.stringify(value)}`
      );
    }
  ...
  assert(ctx: IValidatorContext, value: unknown) {
    ...
    if (isInteger(o.maxProperties) && keys.size <= o.maxProperties) {
      throw ctx.assertionFailure(
        "maxProperties",
        value,
        `Property count of ${JSON.stringify(
          keys.size
        )} is greater than maximum of ${o.maxProperties}`
      );
    }
    if (isInteger(o.minProperties) && keys.size <= o.minProperties) {
      throw ctx.assertionFailure(
        ...
        `Property count of ${JSON.stringify(
          keys.size
        )} is less than minimum of ${o.minProperties}`
      );
    }
              ...
              if (!keys.has(required)) {
                throw ctx.assertionFailure(
                  ...
                  `The property ${JSON.stringify(
                    required
                  )} is required when the property ${JSON.stringify(
                    key
                  )} is set`
                );
              }
  ...
  assert(ctx: IValidatorContext, value: unknown) {
    if (typeof value !== "string") {
      throw ctx.assertionFailure(
        "type",
        value,
        `Expected type to be "string", received typeof ${JSON.stringify(
          typeof value
        )}`
      );
    }
    ...
    if (typeof o.pattern === "string" && o.pattern) {
      ...
      if (!rx.test(value)) {
        throw ctx.assertionFailure(
          "pattern",
          value,
          `String received did not match the required pattern ${JSON.stringify(
            o.pattern
          )}`
        );
      }
  ...
  assert(ctx: IValidatorContext, value: unknown) {
    if (!Array.isArray(value)) {
      throw ctx.assertionFailure(
        "type",
        value,
        `Expected type to be "array", received typeof ${JSON.stringify(
          typeof value
        )}`
      );
    }
      ...
      for (; i < o.items.length; i++) {
        ...
        if (typeof v === "undefined") {
          throw ctx.assertionFailure(
            "items",
            value,
            `Missing required array item at index ${i}`
          );
        }
...
export type CoreSchemaMetaSchema =
  ...
  | boolean;
export const CoreSchemaMetaSchemaCodec: Codec<CoreSchemaMetaSchema> = new Codec<
  CoreSchemaMetaSchema
>(
  new AllOfValidator([
    new ObjectTypeValidator({
      properties: {
        ...
        multipleOf: new NumberTypeValidator({
          type: "number",
          exclusiveMinimum: 0,
        }),
        ...
        maxLength: new DeferredReferenceValidator(
          () => NonNegativeIntegerCodec.validator
        ),
        minLength: new DeferredReferenceValidator(
          () => NonNegativeIntegerDefault0Codec.validator
        ),
        pattern: new StringTypeValidator({}),
        additionalItems: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        items: new ConstantValidator(true),
        maxItems: new DeferredReferenceValidator(
          () => NonNegativeIntegerCodec.validator
        ),
        minItems: new DeferredReferenceValidator(
          () => NonNegativeIntegerDefault0Codec.validator
        ),
        uniqueItems: new BooleanTypeValidator(),
        contains: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        maxProperties: new DeferredReferenceValidator(
          () => NonNegativeIntegerCodec.validator
        ),
        minProperties: new DeferredReferenceValidator(
          () => NonNegativeIntegerDefault0Codec.validator
        ),
        required: new DeferredReferenceValidator(
          () => StringArrayCodec.validator
        ),
        additionalProperties: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        ...
        dependencies: new ObjectTypeValidator({
          additionalProperties: new ConstantValidator(true),
        }),
        propertyNames: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        ...
        type: new ConstantValidator(true),
        format: new StringTypeValidator({}),
        contentMediaType: new StringTypeValidator({}),
        contentEncoding: new StringTypeValidator({}),
        if: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        then: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        else: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
        ...
        not: new DeferredReferenceValidator(
          () => CoreSchemaMetaSchemaCodec.validator
        ),
      },
    }),
...
export const NonNegativeIntegerCodec: Codec<NonNegativeInteger> = new Codec<
  NonNegativeInteger
>(new NumberTypeValidator({ type: "integer", minimum: 0 }));
type NonNegativeIntegerDefault0 = NonNegativeInteger & unknown;
export const NonNegativeIntegerDefault0Codec: Codec<NonNegativeIntegerDefault0> = new Codec<
  NonNegativeIntegerDefault0
>(
  new AllOfValidator([
    new DeferredReferenceValidator(() => NonNegativeIntegerCodec.validator),
    new ConstantValidator(true),
  ])
);
type SchemaArray = CoreSchemaMetaSchema[];
export const SchemaArrayCodec: Codec<SchemaArray> = new Codec<SchemaArray>(
  new ArrayTypeValidator({
    minItems: 1,
    items: new DeferredReferenceValidator(
      () => CoreSchemaMetaSchemaCodec.validator
    ),
  })
);
type StringArray = string[];
export const StringArrayCodec: Codec<StringArray> = new Codec<StringArray>(
  new ArrayTypeValidator({
    uniqueItems: true,
...
type SimpleTypes =
  ...
  | "object"
  | "string";
export const SimpleTypesCodec: Codec<SimpleTypes> = new Codec<SimpleTypes>(
  new ConstantValidator(true)
);
