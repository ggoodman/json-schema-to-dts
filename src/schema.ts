...
class AssertionFailure {
    ...
    constructor(readonly assertion: JSONSchema7Key, readonly message: string, readonly path: string[], readonly value: unknown) { }
}
...
export const CoreSchemaMetaSchemaCodec: Codec<CoreSchemaMetaSchema> = new Codec<CoreSchemaMetaSchema>(new AnyOfValidator([new ObjectTypeValidator({
        properties: {
            ...
            items: new AnyOfValidator([new DeferredReferenceValidator(() => CoreSchemaMetaSchemaCodec.validator), new DeferredReferenceValidator(() => SchemaArrayCodec.validator)]),
            ...
            dependencies: new ObjectTypeValidator({
                additionalProperties: new AnyOfValidator([new DeferredReferenceValidator(() => CoreSchemaMetaSchemaCodec.validator), new DeferredReferenceValidator(() => StringArrayCodec.validator)])
            }),
            ...
            type: new AnyOfValidator([new DeferredReferenceValidator(() => SimpleTypesCodec.validator), new ArrayTypeValidator({
                    minItems: 1,
                    uniqueItems: true,
                    items: new DeferredReferenceValidator(() => SimpleTypesCodec.validator)
                })]),
...
export const NonNegativeIntegerCodec: Codec<NonNegativeInteger> = new Codec<NonNegativeInteger>(new NumberTypeValidator({ "type": "integer", "minimum": 0 }));
type NonNegativeIntegerDefault0 = NonNegativeInteger & unknown;
export const NonNegativeIntegerDefault0Codec: Codec<NonNegativeIntegerDefault0> = new Codec<NonNegativeIntegerDefault0>(new AllOfValidator([new DeferredReferenceValidator(() => NonNegativeIntegerCodec.validator), new ConstantValidator(true)]));
type SchemaArray = (CoreSchemaMetaSchema)[];
export const SchemaArrayCodec: Codec<SchemaArray> = new Codec<SchemaArray>(new ArrayTypeValidator({
    minItems: 1,
    items: new DeferredReferenceValidator(() => CoreSchemaMetaSchemaCodec.validator)
}));
type StringArray = (string)[];
export const StringArrayCodec: Codec<StringArray> = new Codec<StringArray>(new ArrayTypeValidator({
    uniqueItems: true,
    items: new StringTypeValidator({})
}));
type SimpleTypes = "array" | "boolean" | "integer" | "null" | "number" | "object" | "string";
export const SimpleTypesCodec: Codec<SimpleTypes> = new Codec<SimpleTypes>(new ConstantValidator(true));
